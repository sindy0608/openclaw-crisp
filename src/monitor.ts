...
function isLikelyInternalMonologueParagraph(text: string): boolean {
  ...
  const strongMarkers = [
    ...
    /(?:简洁|简短|直接)回复(?:即可|就行|就好|吧|哦)/i,
    /(?:我应该|我需要|我可以|我会|让我先|让我再|让我确认|让我看看|让我判断|让我分析|让我推理|让我思考|让我组织|让我保持|让我梳理|让我列出)[\s\S]{0,30}(?:用户|客户|问题|情况|回复|处理|转人工|知识库|简洁|简洁的回复)/i,
    /(?:用户|客户)(?:之前|刚才|上面|刚刚|最新)(?:已经|说过|问|发|提到|问的是|发送的是|消息)/i,
    /(?:回顾|梳理|总结|整理|罗列|列出|组织)对话历史|(?:回复|输出)要点[：:]/i,
    /^(?:1\.|2\.|3\.|[-*])\s*用户(?:问|发|说|提到|发送)/i,
    /(?:根据|按照|结合)(?:知识库|上下文|历史对话|最新客户消息)/i,
    /(?:不输出|不要输出)任何分析、推理、内部判断/i,
  ];
